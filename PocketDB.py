...
def convert_to_type(str_value, str_type):
    ...
    action_by_str_type = {u'str': unicode,
                          ...
                          u'bool': bool_int,
                          u'classstr': unicode,
                          u'classunicode': unicode,
                          u'classint': int,
                          u'classfloat': float,
                          u'classlong': long,
                          u'classNoneType': none_type,
                          u'classlist': list,
                          u'classbool': bool_str}
...
--дополнительные параметры
    ...
    def get_settings(self):
        ...
        try:
            ...
            return -1
        for row in cur:
            return_value = [row[i] for i in range(2, 6)]
        con.close()
        return return_value

    def reset_settings(self, url_wsdl='', authorization='', login='', password=''):
        con = sqlite3.connect(self.db_name)
        cur = con.cursor()
        cur.executescript("""
            DROP
        ...
        cur.execute("INSERT INTO Settings VALUES (NULL, ?, ?, ?, ?, ?)",
                         (1, url_wsdl, authorization, login, password))
        con.commit()
        con.close()
    ...
    def prepare_send_data(self):
        ...
        if returnvalue == 1:
            return data
        data = [[row[i] for i in range(10)] for row in cur]
        con.close()
        return data
