...
class SimpleObject:
    ...
    def get_info(self, detail=False):
        ...
        kw = ''
        if detail:
            ks = [_ for _ in self.kwargs.keys()]
            ks.sort()
            for k in ks:
                kw += '                    %s: %s \n' % (k, self.kwargs[k])
        return "%s:\n%s" % (self.name, kw)
...
class OnePocket(SimpleObject):
    ...
    def get_info(self, detail=False):
        ...
        if detail:
            ...
            for k in ks:
                kw += '                    %s: %s \n' % (k, self.kwargs[k])
        return f"{self.name}: {self.balance} {self.currency}{kw}"
...
class OneCredit(OnePocket):
    ...
    def get_info(self, detail=False):
        ...
        if detail:
            ...
            for k in ks:
                kw += '                    %s: %s \n' % (k, self.kwargs[k])
        return f'{self.name} ({self.contact}): {self.balance} {self.currency}{kw}'
...
class Pockets:
    ...
    def __init__(self, db_name='MyMoney.db'):
        ...
        self.settings = {'URL': '', 'Authorization': '', 'Login': '', 'Pass': ''}
    ...
    def action_in(self, pocket, item, summ, amount=0, comment=''):
        ...
        if isinstance(pocket, OnePocket):
            ...
            for tpc in self.pockets:
                ...
                allright = False
        if isinstance(item, SimpleObject):
            itm = item
        elif type(item) == str:
            for titm in self.in_items:
                if titm.name == item:
                    itm = titm
                    break
            else:
    ...
    def set_settings(self, url_wsdl, login, password):
        ...
        authorization = 'Basic %s' % base64.encodebytes(f'{login}:{password}'
                                                        .encode()
                                                        ).strip()
        self.settings['Authorization'] = authorization
        self.db.reset_settings(url_wsdl, authorization, login, password)
    ...
    def get_settings(self):
        ...
        if data != -1:
            ...
            self.settings['Login'] = data[2]
            self.settings['Pass'] = data[3]
